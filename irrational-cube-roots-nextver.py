...
st.sidebar.markdown("### 🔍 Decimal Range Filter")
min_root = st.sidebar.number_input("Min Cube Root", value=0.0)
max_root = st.sidebar.number_input("Max Cube Root", value=10.0)
...
filtered_df = full_df[(full_df["cube_root"] >= min_root) & (full_df["cube_root"] <= max_root)]
...
fig = px.histogram(
    ...
    title=f"Cube Roots from {start} to {end} (Filtered: {min_root}–{max_root})"
)
...
st.download_button("Download CSV", data=csv, file_name="filtered_cube_roots.csv", mime="text/csv")
